...
/*!
 * Encapsulates any running process in order make it possible for processes to terminate This
 * wrapper enables the possibility to perfom a few necessary steps after the actual process function
 * has finished.
 */
...
/*!
 * Kills a process by cleaning up the corresponding slot in os_processes. It also calls the garbage
 * collection in order to free any memory that has been allocated by the killed process. Returns
 * True, if the killing process was successful
 *
...
/*!
 * Function that is used by processes to give away their remaining processing time to another
 * process. Their ProcessState is then being set to OS_PS_BLOCKED so the Scheduler ISR knows not to
 * repick the same process.
 */
void os_yield(void) {
    // TODO implement.
}
